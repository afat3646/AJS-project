            ...
            using (MySqlConnection conexion = new MySqlConnection(conexionString))
            {
                ...
                try
                {
                    ...
                    using (MySqlCommand restar_monto = new MySqlCommand(transferencia_bancaria, conexion))
                    {
                        ...
                        if (string.IsNullOrWhiteSpace(No_UsuarioEmiTxtBox.Text) || string.IsNullOrWhiteSpace(No_UsuarioReceptorTxtBox.Text) || string.IsNullOrWhiteSpace(MontoTransferencia_TxtBox.Text))
                        {
                        ...
                        string insert_registro = "INSERT INTO Registro_Trans (No_Cuenta_Emi, Banco, Monto, No_Cuenta_Rec) VALUES (@No_UsuarioEmi, @Banco, @Monto_Trans, @No_UsuarioReceptor)";

                        using (MySqlCommand insertar_registro_trans = new MySqlCommand(insert_registro, conexion))
                        {
                            insertar_registro_trans.Parameters.AddWithValue("@No_UsuarioEmi", No_UsuarioEmiTxtBox.Text);
                            insertar_registro_trans.Parameters.AddWithValue("@Banco", Banco_Txt.Text);
                            insertar_registro_trans.Parameters.AddWithValue("@Monto_Trans", Convert.ToDouble(MontoTransferencia_TxtBox.Text));
                            insertar_registro_trans.Parameters.AddWithValue("@No_UsuarioReceptor", No_UsuarioReceptorTxtBox.Text);

                            insertar_registro_trans.ExecuteNonQuery();
                        }
